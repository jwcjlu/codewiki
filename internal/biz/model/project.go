...
func NewProject(project *ProjectEntity) *Project {

	return &Project{config: &Config{
		Language: project.Language,
		Excludes: strings.Split(project.Excludes, ","),
	},
...
func (p *Project) ParseCode(ctx context.Context, rootPath string) (*Package, error) {
	...
	root := NewPackage(p.shouldInclude, p, p.Project.ID, filepath.Base(rootPath))
...
func (p *Project) GetEntity(pkgName, key string) *Entity {
	...
	if len(p.RootPath) > 0 {
		pkg, ok = p.pkgs[fmt.Sprintf("%s@%s@%s", p.Project.ID, p.RootPath, pkgName)]
	}
	if ok {
		return pkg.GetEntity(key)
	}
...
func (p *Project) AnalyzeExtends(ctx context.Context) {
	p.Root.ClassifyExtends(ctx)
}

// AnalyzeMethod 分析实体方法
func (p *Project) AnalyzeMethod(ctx context.Context) {
	p.Root.ClassifyMethod(ctx)
}

func (p *Project) GetImport() []*Import {
	var imports []*Import
...
func (p *Project) GetFields() []*Field {
	var fields []*Field
	for _, entity := range p.GetEntities() {
		fields = append(fields, entity.GetFields()...)
	}
	return fields
}
func (p *Project) GetEntities() []*Entity {
	var entities []*Entity
	for _, file := range p.GetFiles() {
...
func (p *Project) GetFunctions() []*Function {
