...
import { Repo, RepoTreeResp, Function, CallRelation, Node, NodePosition, PackageNode, FileNode, Language } from '../types';
...
const languageOptions = [
  { label: 'Golang', value: Language.Golang },
...
const RepoManager: React.FC = () => {
  ...
  const [form, setForm] = useState({ name: '', repoType: 0, target: '', path: '', token: '', description: '', language: Language.Golang, excludes: '' });
  ...
  const [viewingFile, setViewingFile] = useState<{ content: string; language: Language; fileName: string; functions: Function[]; highlightFunction?: string } | null>(null);
  // 新增：当前高亮的文件ID
  const [highlightedFileId, setHighlightedFileId] = useState<string | null>(null);
  
  // 新增：调用图相关状态
  const [activeTab, setActiveTab] = useState<'tree' | 'callgraph' | 'mermaid' | 'search'>('tree');
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      await createRepo({
        ...
        excludes: form.excludes ? form.excludes.split(',').map(s => s.trim()).filter(Boolean).join(',') : undefined,
      });
      setForm({ name: '', repoType: 0, target: '', path: '', token: '', description: '', language: Language.Golang, excludes: '' });
      await load();
  ...
  const handleViewFile = async (repoId: string, fileId: string, fileName: string) => {
    try {
      ...
      setViewingFile({
        ...
        language: fileData.body.language as Language,
        fileName: fileName,
      ...
      <div style={{ display: 'grid', gap: 16 }}>
        ...
        <div style={styles.card}>
          ...
          <form onSubmit={handleCreate} style={{ display: 'grid', gap: 10 }}>
            ...
            <div>
              <div style={styles.label}>仓库地址（远端地址或本地路径）</div>
              <input style={styles.input} placeholder="https://github.com/org/repo 或 D:/code/project" value={form.path} onChange={e => setForm({ ...form, path: e.target.value })} required />
            </div>
            <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <div>
                ...
                <select style={styles.input as React.CSSProperties} value={form.language} onChange={e => setForm({ ...form, language: Number(e.target.value) as Language })}>
            ...
            <div style={styles.toolbar}>
              ...
              <button type="button" style={{ ...styles.btn, ...styles.btnGhost }} onClick={() => setForm({ name: '', repoType: 0, target: '', path: '', token: '', description: '', language: Language.Golang, excludes: '' })}>重置</button>
            </div>
          </form>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gap: 16 }}>
        ...
        <div className="tab-container">
          <div className="tab-buttons">
            ...
            <button
              className={`tab-btn ${activeTab === 'callgraph' ? 'active' : ''}`}
              onClick={() => setActiveTab('callgraph')}
            >
              🔗 调用图
            </button>
            <button
              className={`tab-btn ${activeTab === 'mermaid' ? 'active' : ''}`}
          ...
          {activeTab === 'mermaid' && (
            <div className="tab-content mermaid-content">
              <MermaidTest
                ...
                onChartGenerated={(chartType, mermaidCode) => {
                  console.log(`生成了${chartType}类型的图表:`, mermaidCode);
                  // 可以在这里添加图表生成后的处理逻辑
                  // 比如保存到本地、分享等
                }}
              />
