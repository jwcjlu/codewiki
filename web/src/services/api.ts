...
export async function getRepo(id: string): Promise<GetRepoResp> {
  const res = await fetch(`${API_BASE_URL}/repo/${encodeURIComponent(id)}`, { headers: { 'Accept': 'application/json' }, credentials: 'include' });
...
export async function deleteRepo(id: string): Promise<void> {
  const res = await fetch(`${API_BASE_URL}/repo/${encodeURIComponent(id)}`, { method: 'DELETE', credentials: 'include' });
...
export async function analyzeRepo(id: string, forceUpdate: boolean = false): Promise<void> {
  const res = await fetch(`${API_BASE_URL}/repos/${encodeURIComponent(id)}/analyze`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
    credentials: 'include',
    body: JSON.stringify({ id, forceUpdate })
  });
  if (!res.ok) {
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      const eventSource = new EventSource(
        `${API_BASE_URL}/repo/${req.id}/answer?question=${encodeURIComponent(req.question)}`,
